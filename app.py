...
enable_enhancement = st.sidebar.checkbox("Enable Enhancement (CLAHE + WB)", value=True)
clip_limit = st.sidebar.slider("CLAHE Clip Limit", 1.0, 5.0, 2.0, 0.5)
...
model_type = st.sidebar.selectbox(
    ...
    ("models/yolov8n.pt (Fastest)", "models/yolov8s.pt (Balanced)", "models/yolov8m.pt (Accurate)", "models/yolov8l.pt (Most Accurate)")
)

# 4. Mission Controls
st.sidebar.subheader("Mission Controls")
enable_tracking = st.sidebar.checkbox("Enable Tracking (Item 8)", value=True)
enable_calibration = st.sidebar.checkbox("Enable Calibration (Item 11)", value=False)
enable_fallback = st.sidebar.checkbox("Enable Non-AI Fallback (Item 12)", value=False)
enable_logging = st.sidebar.checkbox("Enable Mission Logging (Item 1)", value=True)
enable_ros2 = st.sidebar.checkbox("Enable ROS2 Bridge (Item 10)", value=False)
...
def load_modules(model_name):
    ...
    # Extract filename 
    path = model_name.split(" ")[0]
    detector = TrashDetector(model_path=path) 
...
def process_frame(frame, enhancer, detector, calibrator, fallback, settings):
    ...
    if settings['enable_detection']:
        ...
        # We handle annotation manually to add distance labels
    ...
    if cap and cap.isOpened():
        while st.session_state['run']:
            ...
            if snapshot_placeholder.button("📸 Take Snapshot", key=f"snap_{time.time()}"):
                snap_path = f"snapshot_{int(time.time())}.png"
